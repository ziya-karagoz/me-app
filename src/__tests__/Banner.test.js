...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Ziya' is in the banner component
	expect(screen.getByText(/Hi, Iam Ziya/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Ziya-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Ziya-Resume.pdf');
